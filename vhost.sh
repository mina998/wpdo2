...
source $DNMP_DIR/.env
# SSL目录
SSL_DIR=$NGINX_SSL_CERTIFICATE_DIR/live
# 输入的域名
INPUT_DOMAIN_NAME=""
# 需要操作的站点虚拟主机名
SITE_HOSTNAME=""
# 虚拟主机配置文件
VHOST_CONF_FILE=$(realpath ${DNMP_DIR}/${NGINX_DIR}/vhost.conf)
# 验证域名
...
function create_site {
    ...
    # 修改文件权限 - 使用容器内的路径
    docker exec nginx chown -R www-data:www-data /wwwroot/$INPUT_DOMAIN_NAME
    # 重新加载nginx配置
